...
public final class ExNativeAccessWebcam extends Object
{
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        final JFrame frame = new JFrame();//initialises frame allocates memory for reference of frame
        ...
        content.add(new JLabel("Webcam: ", JLabel.RIGHT));//adds name "webcam " to option present in frame
        content.add(webcamComboBox);
        content.add(new JButton(new AbstractAction("Open") // What os to be done when we click open button
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                new Thread(new Runnable()
                {
                    public void run()
                    {
                        ...
                        //last frame is image that is captured recently from webcam
                        try
                        {
                                ...
                                public void onVideoFrame(final VideoFrame frame)
                                {
                                    SwingUtilities.invokeLater(new Runnable()
                                    {
                                        public void run()
                                        {
                                            ...
                                            capturedImage =VideoFrameFactory.toBufferedImage(frame);
                                            //sets the last image to panel so it will look like video
                            ...
                            webcam.startCapture(); // start current webcam..
        ...
        frame.getContentPane().setLayout(new BorderLayout()); //set frame parameters
        frame.getContentPane().add(content); // adds content to frame
        
        frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        frame.setTitle("Webcam capture options"); //sets title to frame
        ...
        frame.pack();
        frame.setVisible(true); // makes frame visible on current Desktop
    ...
    */
    ...
    private final static class WebcamComboModel extends AbstractListModel implements ComboBoxModel
    {
        ...
        public int getSize()
        {
            return devices.size(); // number of cameras connected to PC
        }
    }
}